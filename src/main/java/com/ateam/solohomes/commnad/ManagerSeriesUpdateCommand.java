	...
	public void execute(final Model model) {
		...
		C.transactionTemplate.execute(new TransactionCallbackWithoutResult() {
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) {
				...
				int sr_uid = (Integer)(map.get("sr_uid"));
				SeriesComponentsDTO oriDto = (SeriesComponentsDTO)map.get("series");
				...
				try {
					...
					result += dao.updateSeries(dto, sr_uid);
					
					dao.deleteSeriesGoodsByUid(sr_uid);
					...
					// 원래 파일 삭제
					File file = new File(SAVE_PATH + "/" + oriDto.getSr_img());
					if (file.exists()) file.delete();
				}
			}
		});
